...
I changed this chunk, in order to have commitee party affiliation for the graph in the next chunch, the summmerize had to be changed to mutate.  This stopped top_n() from working, so I changed it to arrange and head.  Then I thought you could use distinct(sum) to get rid off all of the reapeating sum values, but now its the same problem I was trying to avoid 
...
ggplot(cand_contr_comm, aes(x=name, y=sum))+
  geom_histogram(stat="identity", aes(fill="cmte_party_affiliation")) + 
